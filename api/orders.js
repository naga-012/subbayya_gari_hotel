  ...
  createOrderHandler: (req, res) => {
    try {
      ...
      const newOrder = {
        ...
        branch: (orderData.branch || orderData.branchName || orderData.selectedBranch || 'KPHB Colony, Hyderabad').toString().trim(),
        branchName: (orderData.branchName || orderData.branch || orderData.selectedBranch || 'KPHB Colony, Hyderabad').toString().trim(),
        branchId: (orderData.branchId || ((orderData.branch || orderData.branchName || '').toLowerCase().includes('vanasthal') || (orderData.branch || orderData.branchName || '').toLowerCase().includes('vasanth') ? 'vanasthalipuram' : ((orderData.branch || orderData.branchName || '').toLowerCase().includes('kukat') ? 'kukatpally' : 'kphb'))).toString().trim(),
        branchAddress: orderData.branchAddress || ((orderData.branch || orderData.branchName || '').toLowerCase().includes('vanasthal') || (orderData.branch || orderData.branchName || '').toLowerCase().includes('vasanth') ? 'Plot 14, Sahara Road, Vanasthalipuram, Hyderabad' : ((orderData.branch || orderData.branchName || '').toLowerCase().includes('kukat') ? 'Near Y Junction, Main Road, Kukatpally, Hyderabad' : 'MIG 295, Rd No. 4, KPHB Colony, Kukatpally, Hyderabad')),
